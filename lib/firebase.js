...
export const addImages = async (images, title) => {
  ...
  await Promise.all(
    images.map(file => {
      storage
        .ref(`images/${checkWhiteSpace(title) ? title.toLowerCase().replace(/ /g, "_") : title.toLowerCase()}/${file.name}`)
        .put(file)
        .then((snapshot) => {
          snapshot.ref.getDownloadURL()
            .then(async (url) => {
              projectRef
                .update({ imageURLS:  firebase.firestore.FieldValue.arrayUnion(url)})
            })
        })
    ...
    .then(() => {
      console.log("The URL array is " + urlSubArray);
    });
  ...
  //   storage
  //     .ref(`images/${checkWhiteSpace(title) ? title.toLowerCase().replace(/ /g, "_") : title.toLowerCase()}`)
  //     .listAll()
  //     .then((urls) => {

  //       console.log("The URLS are " + urls.prefixes);
  //       // urls.items.forEach((image) => {
  //       //   image.getDownloadURL()
  //       //     .then(async (url) => {
  //       //       let newURL = url;
  //       //       console.log("Image URL is " + newURL);
  //       //     })
  //       // })
...
export const getImageURLs = async (title, project) => {
  const storage = firebase.storage();
  const urlStrings = [];
  // const urlResult = await 
  // const urlPromises = urlResult.items.map(imageRef => imageRef.getDownloadURL());

  // addProject(title, project, urlStrings);
