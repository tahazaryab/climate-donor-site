...
export const sendToStripe = async (title, id) => {
    fetch("/api/product", {
      method: "POST",
      body: JSON.stringify({
        title: title,
...
export const addProject = async (project, images, title) => {
  ...
  projectRef
    ...
    .then(() => {
      ...
      addImages(images, title, id);
      sendToStripe(title, id);
...
export const addImages = async (images, title, docID) => {
...
export const addStripeId = async (projectId, stripeId) => {
  const db = firebase.firestore();
  ...
  projectRef
    ...
    .then(() => {
      console.log("Stripe price id successfully added.");
      return null;
    })
    .catch((error) => {
