...
export const getUsersDonatedForAProject = async (projectId) =>{
  ...
  return snapshot.docs.map(async doc=> (await getDoc("users", doc.data().userId.id)).data())
}
...
export const getUserDonatedProjects = async(userId) => {
  ...
  var snapshot = await firebase.firestore()
    ...
    .get()
  return snapshot.docs.map(async doc=> (await getDoc("projects", doc.data().projectId.id)).data())
}
